...
export function activate(context: vscode.ExtensionContext) {
  ...
  let marksProviderView = vscode.window.createTreeView("explorer-marks", {
    treeDataProvider: marksProvider,
    // showCollapseAll: true,
    // canSelectMany: true,
    // dragAndDropController: new TestViewDragAndDrop(context),
  });
  marksProvider.setView(marksProviderView);
  let refreshEntry = async () => {
  ...
  let addEntryFunc = async () => {
    ...
    if (filePath !== undefined && line !== undefined && content !== undefined) {
      await marksProvider.addMark(filePath, line, content);
    }
  };
  ...
  let collapsedAll = async () => {
    await marksProvider.collapsedAll();
  };
  let expandedAll = async () => {
    await marksProvider.expandedAll();
  };
  ...
  vscode.commands.registerCommand("explorer-marks.collapsedAll", collapsedAll);
  vscode.commands.registerCommand("explorer-marks.expandedAll", expandedAll);
